...
=== Random Paragraph
(This just popped up down below, not sure it's useful) As mentioned earlier, there's a lot more to Big O than what's in this chapter, but you're not too likely to be asked about it in a coding interview.
...
O(latexmath:[n^2]) (also called "quadratic") means that every operation must be done the same number of times as the number of inputs.
...
This doesn't mean your car will always get this exact gas milage.
...
This is a not a book about asymptotic analysis, and there are lot of them out there, so let's stick with Big O and the four shapes we considered earlier.
